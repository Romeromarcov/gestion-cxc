"""
Módulo Interno — Ventas Internas
Ventas de productos locales (no registradas en Odoo).
...
def _require_modulo(con=None):
    """Lanza 403 si el módulo interno está desactivado en config_app."""
    _con = con or get_con()
    row = _con.execute(
...
def crear_venta(body: VentaInternaCreate, user=Depends(get_current_user)):
    ...
    cur = con.execute("""
        INSERT INTO ventas_internas
            (codigo, cliente_nombre, cliente_id_odoo, vendedor_id, notas)
        VALUES (?,?,?,?,?)
...
def agregar_linea(venta_id: int, body: LineaVentaInterna,
    ...
    # Recalcular total
    total = con.execute("""
        SELECT COALESCE(SUM(cantidad * precio_unitario * (1 - descuento_pct/100)), 0)
...
@router.delete('/{venta_id}/linea/{linea_id}')
def eliminar_linea(venta_id: int, linea_id: int,
                   user=Depends(get_current_user)):
    con = get_con()
    _require_modulo(con)
    venta = row_to_dict(con.execute(
        "SELECT estado FROM ventas_internas WHERE id=?", (venta_id,)
    ).fetchone())
    if not venta or venta['estado'] != 'borrador':
        con.close()
        raise HTTPException(status_code=400, detail='Solo se pueden eliminar líneas de ventas en borrador')
    con.execute(
        "DELETE FROM ventas_internas_lineas WHERE id=? AND venta_id=?",
        (linea_id, venta_id)
    )
    total = con.execute("""
        SELECT COALESCE(SUM(cantidad * precio_unitario * (1 - descuento_pct/100)), 0)
    ...
    con.commit()
    con.close()
    return {'mensaje': 'Línea eliminada', 'total_usd': total}
...
def confirmar_venta(venta_id: int, user=Depends(get_current_user)):
    ...
    if not lineas:
        con.close()
        raise HTTPException(status_code=400, detail='La venta no tiene líneas')

    # Descontar inventario automáticamente (no puede quedar negativo)
    for linea in lineas:
        con.execute("""
            UPDATE inventario_interno
            SET stock_actual = GREATEST(0, stock_actual - ?)
            WHERE producto_codigo=?
        """, (linea['cantidad'], linea['producto_codigo']))
